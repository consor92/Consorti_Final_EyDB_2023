...
{
    menuopciones sm[]={"","CLIENTE","PROVEEDOR","TODOS","<ESC>"};
...
{
    cliente_proveedor cp_aux;

    cliente_proveedor cp;
    menuopciones sm[]={"","DOCUMENTO","RAZON SOC","EMPRESA","CONTACTO"};
    int rango=sizeof(sm)/27,menu;
    do
    {
        system("cls");
        strcpy(cp.id_empresa,"");
        submenu(&menu,sm,rango,6);
        gets(cp_aux.contacto_nombre);
        fflush(stdin);
        ;
